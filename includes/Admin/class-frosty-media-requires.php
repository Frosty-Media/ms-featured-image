...
/**
 * Everything this plugin requires.
 *
 * @since	12/13/2014
 */ 
if ( ! class_exists( 'Frosty_Media_Requires' ) ) {

    /**
     * Make sure the Frosty Media plugin exists.
     *
     * @since 1.0.1
     *
     * @package Frosty-Media-Requires
     * @author  Austin Passy <austin.passy.co>
     */
	class Frosty_Media_Requires {
		...
		public static function instance() {
			if ( ! isset( self::$instance ) && ! ( self::$instance instanceof Frosty_Media_Requires ) ) {
				self::$instance = new Frosty_Media_Requires;
			}
		...
		private function add_actions() {			
			...
			add_action( 'init',				array( $this, 'frosty_media_license_loader' ) );
		}
		
		/**
		 * Register our required plugins.
		 */
		function tgmpa_register() {
			...
			$plugins = array(
				array(
					'name'			=> 'Frosty Media (License Manager &amp; Auto-updates)', // The plugin name.
					'slug'			=> 'frosty-media', // The plugin slug (typically the folder name).
					'source'		=> 'https://github.com/Frosty-Media/frosty-media/archive/master.zip', // The plugin source.
					'required' 		=> true, // If false, the plugin is only 'recommended' instead of required.
					'external_url'	=> 'https://github.com/Frosty-Media/frosty-media', // If set, overrides default API URL and points to an external URL.
				),
			);
			
			$plugins		= apply_filters( 'frosty_media_requires_plugins_array', $plugins );
			$domain			= apply_filters( 'frosty_media_requires_text_domain', 'frosty-media-requires' );
			
			/**
			 * Array of configuration settings. Amend each line as needed.
			 * If you want the default strings to be available under your own plugin domain,
			 * leave the strings uncommented.
			 * Some of the strings are added into a sprintf, so see the comments at the
			 * end of each line for what each argument will be.
			 */
			$config = array(
				'domain'       		=> $domain,         				// Text domain - likely want to be the same as your plugin.
		...
		/**
		 * Let's do some sort of license page incase the user doesn't want to
		 * install the 'Frosty_Media' class from GitHub.
		 *
		 * @since	12/26/2014
		 */
		function frosty_media_license_loader() {
    ...
    $Frosty_Media_Requires = Frosty_Media_Requires::instance();
}
